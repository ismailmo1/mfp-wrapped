...
def generate_total_kcal_card(num_kcal: int):
    ...
    title_fnt = ImageFont.truetype(FONT_PATH, 90)
    draw.text(
        (5, (height / 2) - 180),
        f"{num_kcal:,}",
        font=title_fnt,
    ...
    home_img.thumbnail((100, 80))

    home_imgs = [home_img for _ in range(math.ceil(num_homes))]
    fraction = num_homes % 1
    if fraction > 0:
        width = home_img.size[0]
        home_imgs[-1] = home_img.crop((0, 0, int(width * fraction), 100))
    # TODO: arrange grid and resize based on num home_imgs
    for idx, home in enumerate(home_imgs):
        card.paste(home, (20 + (80 * idx), 420), home)
...
def generate_adherence_card(adherence: float):
    ...
    adherence_card.paste(meme_img_border, (90, 80))
    phrase_font = ImageFont.truetype(FONT_PATH, 18)

    draw.text(
        (80, 270),
        phrase_imgs[threshold_lvl][0],
        font=phrase_font,
        align="center",
    )
